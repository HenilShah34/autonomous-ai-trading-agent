...
USER_DATA_DIR = "user_data_crude"
URL = "https://www.investing.com/commodities/crude-oil-streaming-chart"
...
def read_and_store_candle(tv, timeframe, ts):
    ...
    insert_market_data("crude", data)
    print("📥 Candle inserted:", data)


# -------------------------------------------------
# READ + INSERT LIVE CRUDE PRICE (SEPARATE ROW)
# -------------------------------------------------
def read_and_store_crude_price(page, ts):
    try:
        ...
        insert_market_data("crude", data)
        print(f"💰 Crude Price inserted: {price}")

    except Exception:
        print("⚠️ Crude price unavailable")
...
with sync_playwright() as p:
    ...
    print("\n🔁 Starting OHLC + MA + Crude Price loop...\n")

    while True:
        ts = datetime.now(timezone.utc)

        # ✅ LIVE CRUDE PRICE ROW
        read_and_store_crude_price(page, ts)
