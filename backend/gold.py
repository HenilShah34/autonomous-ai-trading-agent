...
def read_and_store_candle(tv, timeframe, ts):
    ...
    insert_market_data("gold", data)
    print("📥 Candle inserted:", data)


# -------------------------------------------------
# READ + INSERT LIVE GOLD PRICE (SEPARATE ROW)
# -------------------------------------------------
def read_and_store_gold_price(page, ts):
    try:
        ...
        insert_market_data("gold", data)
        print(f"💰 GOld Price inserted: {price}")

    except Exception:
        print("⚠️ Gold price unavailable")
...
with sync_playwright() as p:
    ...
    print("\n🔁 Starting OHLC + MA + Gold Price loop...\n")

    while True:
        ts = datetime.now(timezone.utc)

        # ✅ LIVE GOLD PRICE ROW
        read_and_store_gold_price(page, ts)
