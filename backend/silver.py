...
USER_DATA_DIR = "user_data_silver"
...
def read_and_store_candle(tv, timeframe, ts):
    ...
    insert_market_data("silver", data)
    print("📥 Candle inserted:", data)


# -------------------------------------------------
# READ + INSERT LIVE SILVER PRICE (SEPARATE ROW)
# -------------------------------------------------
def read_and_store_silver_price(page, ts):
    try:
        ...
        insert_market_data("silver", data)
        print(f"💰 Silver Price inserted: {price}")

    except Exception:
        print("⚠️ Silver price unavailable")
...
with sync_playwright() as p:
    ...
    print("\n🔁 Starting OHLC + MA + Silver Price loop...\n")

    while True:
        ts = datetime.now(timezone.utc)

        # ✅ LIVE SILVER PRICE ROW
        read_and_store_silver_price(page, ts)
